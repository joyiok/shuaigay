  ...
  return (
    <div style={{ maxWidth: 460, margin: "32px auto", padding: "0 8px" }}>
      <div className="card" style={{ padding: 28 }}>
        ...
        {inviteCode && (
          <div style={{ background: "var(--brand-soft)", border: "1.5px solid var(--line)", borderRadius: 10, padding: "10px 12px", fontSize: 12, marginBottom: 12, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <span style={{ color: "var(--text-muted)" }}>受邀注册</span>
            <strong style={{ letterSpacing: "0.06em", background: "var(--panel)", border: "1px solid var(--line)", padding: "2px 8px", borderRadius: 999 }}>{inviteCode}</strong>
          </div>
        )}

        <form action={registerAction} style={{ display: "grid", gap: 18 }}>
        ...
        <div style={{ marginTop: 12, padding: "10px 12px", background: "var(--bg-soft)", border: "1px dashed var(--line-soft)", borderRadius: 10, fontSize: 12, color: "var(--text-subtle)", lineHeight: 1.5, textAlign: "center" }}>
      ...
      <p style={{ textAlign: "center", fontSize: 12, color: "var(--text-subtle)", marginTop: 10, }}>注册即表示你同意当自己家，别端着</p>
    </div>
  );
}
