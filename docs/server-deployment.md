...
站点自带五位数字验证码，不需要配置 Cloudflare Turnstile 或任何第三方验证码密钥。首次部署默认开启，覆盖登录、注册、发主题和回复；管理员可在「站点设置 → 安全验证」随时开关。

验证码存入 Redis，五分钟内有效且只能使用一次。开启状态下 Redis 不可用时会拒绝验证，避免绕过；Compose 已包含 Redis 服务。`CAPTCHA_TEST_MODE=1` 只用于自动化测试，生产配置检查会拒绝该值。
